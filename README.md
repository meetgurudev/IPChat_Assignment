...
The directory created for you on each of the 5 dedicated hosts. Change the access permission to this directory so that only you are allowed to access the contents of the directory. This is to prevent others from getting access to your code.
...
```
IP_ChatGroups_print_and_log(char* format, ...)
```
...
IP_ChatGroups_print_and_log(“[%s:SUCCESS]\n”, command_str);

IP_ChatGroups_print_and_log(“IP:%s\n”, ip_addr);

IP_ChatGroups_print_and_log(“[%s:END]\n”, command_str);

//Error

IP_ChatGroups_print_and_log(“[%s:ERROR]\n”, command_str);
...
```
1 | xxx.xxx.flud.vedo | xxx.205.XX.XX | 4545

2 | emba.xxx.flud.vedo | 128.xxx.XX.XX | 5000

3 | higate.xxx.flud.vedo | 128.XX.xx.33 | 5499

4 | euxxx.xxx.flud.vedo |  XXX.205.XX.xx | 5701
```
...
#### 5.7 Peer-to-peer (P2P) file transfer
...
To transfer both text and binary files. You can assume the maximum file size to be 10 MB.
